...
class TfExampleDecoderTest(tf.test.TestCase):
    ...
    def testDecodeJpegImage(self):
        ...
        example = tf.train.Example(features=tf.train.Features(feature={
            ...
            'image/source_id': self._BytesFeature('image_id'.encode('utf8')),
        })).SerializeToString()
    ...
    def testDecodeImageKeyAndFilename(self):
        ...
        example = tf.train.Example(features=tf.train.Features(feature={
            'image/encoded': self._BytesFeature(encoded_jpeg),
            'image/key/sha256': self._BytesFeature('abc'.encode('utf8')),
            'image/filename': self._BytesFeature('filename'.encode('utf8'))
        })).SerializeToString()
    ...
    def testDecodePngImage(self):
        ...
        example = tf.train.Example(features=tf.train.Features(feature={
            ...
            'image/format': self._BytesFeature('png'.encode('utf8')),
            'image/source_id': self._BytesFeature('image_id'.encode('utf8'))
        })).SerializeToString()
    ...
    def testDecodeRotatedBoundingBox(self):
        ...
        self.assertAllEqual((tensor_dict[fields.InputDataFields.groundtruth_rboxes].get_shape().as_list()), [None, 5])
        with self.test_session() as sess:
            tensor_dict = sess.run(tensor_dict)

        expected_rboxes = np.vstack([rbbox_cy, rbbox_cx, rbbox_h, rbbox_w, rbbox_ang]).transpose().astype(np.float32)
        self.assertAllEqual(expected_rboxes, tensor_dict[fields.InputDataFields.groundtruth_rboxes])
