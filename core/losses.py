...
class HardExampleMiner(object):
    ...
    def __init__(self,
        ...
        """
        if box_type != 'bbox' and box_type != 'rbbox':
            raise ValueError("box_type have to be 'bbox' or 'rbbox'.")
    ...
    def __call__(self,
        ...
        for ind, detection_boxlist in enumerate(decoded_boxlist_list):
            ...
            if self._box_type == 'bbox':
                selected_indices = tf.image.non_max_suppression(box_locations,
                                                                image_losses,
                                                                num_hard_examples,
                                                                self._iou_threshold)
            elif self._box_type == 'rbbox':
                # nms is't supported
                num_boxes = tf.minimum(tf.shape(image_losses)[0], num_hard_examples)
                _, selected_indices = tf.nn.top_k(image_losses, num_boxes)

            if self._max_negatives_per_positive is not None and match:
                (selected_indices, num_positives, num_negatives) \
                    = self._subsample_selection_to_desired_neg_pos_ratio(selected_indices,
                                                                         match,
    ...
    def _subsample_selection_to_desired_neg_pos_ratio(self,
        ...
        topk_negatives_indicator = tf.less_equal(tf.cumsum(tf.to_int32(negatives_indicator)),
                                                 max_negatives)
        # to exclude the ignore indicator
        topk_negatives_indicator = tf.logical_and(topk_negatives_indicator, negatives_indicator)
        subsampled_selection_indices = tf.where(tf.logical_or(positives_indicator,
