...
class MaskRCNNBoxPredictorTest(tf.test.TestCase):
    ...
    def test_get_boxes_with_five_classes_roi_pooling(self):
        image_features = tf.random_uniform([4, 7, 7, 3], dtype=tf.float32)
        mask_box_predictor = box_predictor.MaskRCNNBoxPredictor(
            is_training=False,
            num_classes=5,
            fc_hyperparams=self._build_arg_scope_with_hyperparams(),
            ...
            dropout_keep_prob=0.5,
            box_code_size=4,
            roi_pooling=rfaster_rcnn_pb2.RfasterRcnn.CRRoIxDF
        )
        box_predictions = mask_box_predictor.predict(
            image_features, num_predictions_per_location=1, scope='BoxPredictor')
        box_encodings = box_predictions[box_predictor.BOX_ENCODINGS]
    ...
    def test_get_boxes_with_five_classes_with_auxiliary(self):
        image_features = tf.random_uniform([2, 7, 7, 3], dtype=tf.float32)
        auxiliary = tf.random_uniform([2, 7], dtype=tf.float32)
        mask_box_predictor = box_predictor.MaskRCNNBoxPredictor(
            is_training=False,
            num_classes=5,
            fc_hyperparams=self._build_arg_scope_with_hyperparams(),
        ...
        box_predictions = mask_box_predictor.predict(
            image_features, num_predictions_per_location=1, auxiliary=auxiliary, scope='BoxPredictor')
        box_encodings = box_predictions[box_predictor.BOX_ENCODINGS]
        class_predictions_with_background = box_predictions[
            box_predictor.CLASS_PREDICTIONS_WITH_BACKGROUND]
        init_op = tf.global_variables_initializer()
