...
class TargetAssigner(object):
    ...
    def assign(self, anchors, groundtruth_boxes, groundtruth_labels=None,
        ...
        Args:
          anchors: a BoxList or a RBoxList representing N anchors
          groundtruth_boxes: a BoxList or RBoxList representing M groundtruth boxes
        ...
        Raises:
          ValueError: if anchors or groundtruth_boxes are not of type
            box_list.BoxList
        """
        if not (isinstance(anchors, box_list.BoxList) or isinstance(anchors, rbox_list.RBoxList)):
            raise ValueError('anchors must be an BoxList or a RBoxList')
        if not (isinstance(groundtruth_boxes, box_list.BoxList) or isinstance(groundtruth_boxes, rbox_list.RBoxList)):
    ...
    def _create_regression_targets(self, anchors, groundtruth_boxes, match):
        ...
        Args:
          anchors: a BoxList or a RBoxList representing N anchors
          groundtruth_boxes: a BoxList or a RBoxList representing M groundtruth_boxes
          match: a matcher.Match object

        Returns:
          reg_targets: a float32 tensor with shape [N, box_code_dimension]
        """
        if isinstance(anchors, rbox_list.RBoxList):
            gather = rbox_list_ops.gather
        elif isinstance(anchors, box_list.BoxList):
            gather = box_list_ops.gather
        else:
            raise ValueError('anchors must be an BoxList or a RBoxList')
        ...
        matched_anchors = gather(anchors, matched_anchor_indices)
        matched_gt_boxes = gather(groundtruth_boxes, matched_gt_indices)
...
def create_target_assigner(reference, stage=None,
    ...
    if reference == 'Multibox' and stage == 'proposal':
        ...
        similarity_calc = sim_calc.IouSimilarity()
        # Uses all proposals with IOU < 0.5 as candidate negatives.
        matcher = argmax_matcher.ArgMaxMatcher(matched_threshold=0.4,
                                               negatives_lower_than_unmatched=True,
                                               ignore_sim_less_than_zero=True)
        box_coder = faster_rcnn_rbox_coder.FasterRcnnRBoxCoder(
            scale_factors=[10.0, 10.0, 5.0, 5.0, 5.0])
...
def batch_assign_targets(target_assigner,
    ...
    Args:
      ...
      anchors_batch: BoxList or RBoxlist representing N box anchors or list of BoxList objects
        with length batch_size representing anchor sets.
      gt_box_batch: a list of BoxList or RBoxList objects with length batch_size
    ...
    if not all(isinstance(anchors, box_list.BoxList) or isinstance(anchors, rbox_list.RBoxList)
    ...
    return batch_cls_targets, batch_cls_weights, batch_reg_targets, batch_reg_weights, match_list
