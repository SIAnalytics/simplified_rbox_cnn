...
def random_horizontal_flip_rbox(
        image,
        rboxes=None,
        seed=None):
    """Randomly decides whether to flip horizontally the image and detections or not.

    The probability of flipping the image is 50%.

    Args:
      image: rank 3 float32 tensor with shape [height, width, channels].
      rboxes: (optional) rank 2 float32 tensor with shape [N, 5] containing the bounding boxes.
             Boxes are in normalized form meaning their coordinates vary between [0, 1].
             Each row is in the form of [cy, cx, w, h, angle].
      seed: random seed

    Returns:
      image: image which is the same shape as input image.

      If boxes is not None, the function also returns the following tensors.

      boxes: rank 2 float32 tensor containing the bounding boxes -> [N, 5].
             Boxes are in normalized form meaning their coordinates vary
             between [0, 1].
    """
    def _flip_horizontal_rboxex(rboxes):
        cy, cx, h, w, rad = tf.split(value=rboxes, num_or_size_splits=5, axis=1)
        flipped_cx = tf.subtract(1.0, cx)
        flipped_boxes = tf.concat([cy, flipped_cx, h, w, -rad], 1)

        return flipped_boxes

    def _flip_left_right_image(image):
        # flip image
        image_flipped = tf.image.flip_left_right(image)
        return image_flipped

    with tf.name_scope('RandomHorizontalFlip_rbox', values=[image, rboxes]):
        result = []
        # random variable defining whether to do flip or not
        do_a_flip_random = tf.random_uniform([], seed=seed)
        # flip only if there are bounding boxes in image!
        do_a_flip_random = tf.logical_and(tf.greater(tf.size(rboxes), 0), tf.greater(do_a_flip_random, 0.5))

        # flip image
        image = tf.cond(do_a_flip_random, lambda: _flip_left_right_image(image), lambda: image)
        result.append(image)

        # flip boxes
        if rboxes is not None:
            rboxes = tf.cond(do_a_flip_random, lambda: _flip_horizontal_rboxex(rboxes), lambda: rboxes)
            result.append(rboxes)

        return tuple(result)


def random_vertical_flip_rbox(
        image,
        rboxes=None,
        seed=None):
    """Randomly decides whether to flip vertically the image and detections or not.
    ...
    def _flip_vertical_rboxes(rboxes):
        cy, cx, h, w, rad = tf.split(value=rboxes, num_or_size_splits=5, axis=1)
        flipped_cy = tf.subtract(1.0, cy)
        flipped_boxes = tf.concat([flipped_cy, cx, h, w, -rad], 1)

        return flipped_boxes

    def _flip_up_down_image(image):
        # flip image
        image_flipped = tf.image.flip_up_down(image)
        return image_flipped

    with tf.name_scope('RandomHorizontalFlip', values=[image, rboxes]):
        result = []
        # random variable defining whether to do flip or not
        do_a_flip_random = tf.random_uniform([], seed=seed)
        # flip only if there are bounding boxes in image!
        ...
        # flip image
        image = tf.cond(do_a_flip_random, lambda: _flip_up_down_image(image), lambda: image)
        result.append(image)

        # flip boxes
        if rboxes is not None:
            rboxes = tf.cond(do_a_flip_random, lambda: _flip_vertical_rboxes(rboxes), lambda: rboxes)
            result.append(rboxes)

        return tuple(result)


def random_rotate_rbox(
        image,
        rboxes=None,
        min_value=-math.pi / 18,
        max_value=math.pi / 18,
        by_90=False,
        seed=None):
    """Randomly decides whether to rotate the image and detections or not.

    The probability of rotating the image is 50%.
    The rotation range is between min_value and max_value[default -10' ~ 10'].

    Args:
      image: rank 3 float32 tensor with shape [height, width, channels].
      rboxes: (optional) rank 2 float32 tensor with shape [N, 5]
             containing the bounding rboxes.
             Boxes are in normalized form meaning their coordinates vary between [0, 1].
             Each row is in the form of [cy, cx, w, h, angle].
      min_value: minimum value of the random rotation angle.
      max_value: maximum value of the random rotation angle.
      by_90: whether to rotate by 90 degree
      seed: random seed
    ...
    # rotation boxes
    def _rotate_rbox(rboxes, rot_rad, image_rotated_shape):
        if rboxes is not None:
            image_shape = tf.cast(tf.shape(image), dtype=tf.float32)
            img_cy = image_shape[0] / 2
            ...
            r_cx = img_cx + (cx - img_cx) * tf.cos(rot_rad) - (cy - img_cy) * tf.sin(rot_rad)
            r_cy = img_cy + (cx - img_cx) * tf.sin(rot_rad) + (cy - img_cy) * tf.cos(rot_rad)

            # When rotated by 90, image size is changed. Therefore center points have to be adjusted.
            image_rotated_shape = tf.cast(image_rotated_shape, dtype=tf.float32)
            r_cy += (image_rotated_shape[0] - image_shape[0]) / 2
            r_cx += (image_rotated_shape[1] - image_shape[1]) / 2

            r_cy = r_cy / image_rotated_shape[0]
            r_cx = r_cx / image_rotated_shape[1]
            h = h * image_shape[0] / image_rotated_shape[0]
            w = w * image_shape[1] / image_rotated_shape[1]

            r_rad = rad + rot_rad
            r_rad = tf.where(r_rad > (math.pi / 2), r_rad - math.pi, r_rad)
            r_rad = tf.where(r_rad < (-math.pi / 2), r_rad + math.pi, r_rad)

            r_rboxes = tf.concat([r_cy, r_cx, h, w, r_rad], 1)
            return r_rboxes

    with tf.name_scope('RandomRotate_rbox', values=[image, rboxes]):
        result = []
        # random variable defining whether to do flip or not
        do_rotate_random = tf.random_uniform([], seed=seed)
        # flip only if there are bounding boxes in image!
        do_rotate_random = tf.logical_and(tf.greater(tf.size(rboxes), 0), tf.greater(do_rotate_random, 0.5))

        # rotate image
        if by_90:
            k = tf.random_uniform([], minval=1, maxval=4, dtype=tf.int32, seed=seed)
            rot_rad = tf.where(tf.equal(k, 3), -math.pi / 2, tf.cast(k, tf.float32) * math.pi / 2)
            image_rotated = tf.cond(do_rotate_random,
                                    lambda: tf.image.rot90(image, -k),
                                    lambda: image)
        else:
            rot_rad = tf.random_uniform([], minval=min_value, maxval=max_value, dtype=tf.float32, seed=seed)
            image_rotated = tf.cond(do_rotate_random,
                                    lambda: tf.contrib.image.rotate(image, -rot_rad, interpolation='BILINEAR'),
                                    lambda: image)
        result.append(image_rotated)

        # rotate rboxes
        if rboxes is not None:
            boxes_rotated = tf.cond(do_rotate_random, lambda: _rotate_rbox(rboxes, rot_rad, tf.shape(image_rotated)),
                                    lambda: rboxes)
            result.append(boxes_rotated)
...
def get_default_func_arg_map(include_instance_masks=False,
    ...
    prep_func_arg_map = {
        ...
        ssd_random_crop_fixed_aspect_ratio: (
            ...
            groundtruth_instance_masks,
            groundtruth_keypoints,),
        random_horizontal_flip_rbox: (fields.InputDataFields.image,
                                      fields.InputDataFields.groundtruth_rboxes,),
        random_vertical_flip_rbox: (fields.InputDataFields.image,
                                    fields.InputDataFields.groundtruth_rboxes,),
        random_rotate_rbox: (fields.InputDataFields.image,
                             fields.InputDataFields.groundtruth_rboxes,),
    }
