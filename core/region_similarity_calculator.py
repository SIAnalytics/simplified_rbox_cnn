...
class IouSimilarity(RegionSimilarityCalculator):
    ...
    def _compare(self, boxlist1, boxlist2):
        ...
        """
        if isinstance(boxlist1, rbox_list.RBoxList) and isinstance(boxlist2, rbox_list.RBoxList):
            return rbox_list_ops.iou(boxlist1, boxlist2)
        elif isinstance(boxlist1, box_list.BoxList) and isinstance(boxlist2, box_list.BoxList):
            return box_list_ops.iou(boxlist1, boxlist2)
        else:
            raise ValueError('boxlist type must be same and BoxList or RBoxsList.')
