...
def multiclass_non_max_suppression_rbox(boxes,
    ...
    with tf.name_scope(scope, 'MultiClassNonMaxSuppression'):
        ...
        length_assert = tf.Assert(tf.equal(num_boxes, num_scores),
                                  ['Incorrect scores field length: actual vs expected.', num_scores, num_boxes])
        ...
        if handle_as_single_class and num_classes > 1:
            ...
            indices = tf.concat([tf.expand_dims(boxs_range, axis=1), tf.expand_dims(max_idxs, axis=1)], axis=1)
        ...
        for class_idx, boxes_idx in zip(range(num_classes), boxes_ids):
            ...
            if is_run_handle_as_single_class:
                ...
                boxlist_and_class_scores.add_field(fields.BoxListFields.classes, max_idxs)
            ...
            rboxlist_filtered = rbox_list_ops.filter_greater_than(boxlist_and_class_scores, score_thresh)
            if clip_window is not None:
                # Not implemented
                # boxlist_filtered = rbox_list_ops.clip_to_window(boxlist_filtered, clip_window)
                if change_coordinate_frame:
                    rboxlist_filtered = rbox_list_ops.change_coordinate_frame(rboxlist_filtered, clip_window)
            max_selection_size = tf.minimum(max_size_per_class, rboxlist_filtered.num_boxes())

            # TODO: support frozen graph
            nms_result = rbox_list_ops.non_max_suppression(rboxlist_filtered,
                                                           iou_thresh,
                                                           max_selection_size,
            ...
            if not is_run_handle_as_single_class:
                nms_result.add_field(fields.BoxListFields.classes,
                                     (tf.zeros_like(nms_result.get_field(fields.BoxListFields.scores)) + class_idx))
            selected_boxes_list.append(nms_result)

        selected_boxes = rbox_list_ops.concatenate(selected_boxes_list)
        sorted_boxes = rbox_list_ops.sort_by_field(selected_boxes, fields.BoxListFields.scores)
        if max_total_size:
            max_total_size = tf.minimum(max_total_size, sorted_boxes.num_boxes())
            sorted_boxes = rbox_list_ops.gather(sorted_boxes, tf.range(max_total_size))
        return sorted_boxes


def batch_multiclass_non_max_suppression_rbox(boxes,
                                              scores,
                                              score_thresh,
                                              iou_thresh,
                                              max_size_per_class,
    ...
    Args:
      boxes: A [batch_size, num_anchors, q, 5] float32 tensor containing
      ...
      num_valid_boxes: (optional) a Tensor of type `int32`. A 1-D tensor of shape
        [batch_size] representing the number of valid boxes to be considered
          ...
          paddings.
      handle_as_single_class: (optional) Bool.
        if True, multi classes are not considered and all are treated as the same label.
    ...
    Returns:
      'nmsed_boxes': A [batch_size, max_detections, 5] float32 tensor
    ...
    with tf.name_scope(scope, 'BatchMultiClassNonMaxSuppression'):
        ...
        def single_image_nms_fn(args):
            ...
            per_image_boxes = tf.reshape(tf.slice(per_image_boxes, 3 * [0],
                                                  tf.stack([per_image_num_valid_boxes, -1, -1])), [-1, q, 5])
            per_image_scores = tf.reshape(tf.slice(per_image_scores, [0, 0],
                                                   tf.stack([per_image_num_valid_boxes, -1])), [-1, num_classes])

            per_image_masks = tf.reshape(tf.slice(per_image_masks, 4 * [0],
                                                  tf.stack([per_image_num_valid_boxes, -1, -1, -1])),
                                         [-1, q, per_image_masks.shape[2].value,
                                          per_image_masks.shape[3].value])

            nmsed_boxlist = multiclass_non_max_suppression_rbox(
                per_image_boxes,
                per_image_scores,
                score_thresh,
                iou_thresh,
                ...
                handle_as_single_class=handle_as_single_class,
                clip_window=clip_window,
                change_coordinate_frame=change_coordinate_frame,
                intersection_tf=intersection_tf)

            padded_boxlist = rbox_list_ops.pad_or_clip_box_list(nmsed_boxlist, max_total_size)
