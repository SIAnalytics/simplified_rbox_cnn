...
"""RSSD Meta-architecture definition.
This code referred to 'ssd_meta_arch.py'.

General tensorflow implementation of convolutional Multibox/SSD detection models.
"""
...
class RSSDMetaArch(model.DetectionModel):
    """RSSD Meta-architecture definition."""
    ...
    def __init__(self,
        ...
        """
        super(RSSDMetaArch, self).__init__(num_classes=box_predictor.num_classes)
    ...
    def anchors(self):
        ...
        if not isinstance(self._anchors, rbox_list.RBoxList):
            raise RuntimeError('anchors should be a RBoxList object, but is not.')
    ...
    def predict(self, preprocessed_inputs):
        ...
        populated with a rbox_list.RBoxList of anchors.  These anchors must be
    ...
    def postprocess(self, prediction_dict):
        ...
        with tf.name_scope('Postprocessor'):
            ...
            (nmsed_boxes, nmsed_scores, nmsed_classes, _, num_detections)\
    ...
    def loss(self, prediction_dict, scope=None):
        ...
        with tf.name_scope(scope, 'Loss', prediction_dict.values()):
            (batch_cls_targets, batch_cls_weights, batch_reg_targets, batch_reg_weights, match_list) \
                = self._assign_targets(self.groundtruth_lists(fields.BoxListFields.boxes),
            ...
            if self._hard_example_miner:
                (localization_loss, classification_loss) = self._apply_hard_mining(location_losses,
                                                                                   cls_losses,
                                                                                   prediction_dict,
                                                                                   match_list)
                if self._add_summaries:
    ...
    def _assign_targets(self, groundtruth_boxes_list, groundtruth_classes_list):
        ...
        return target_assigner.batch_assign_targets(self._target_assigner,
                                                    self.anchors,
                                                    groundtruth_boxlists,
                                                    groundtruth_classes_with_background_list)
    ...
    def _apply_hard_mining(self, location_losses, cls_losses, prediction_dict, match_list):
        ...
        for box_location, box_score in zip(decoded_box_tensors_list, class_prediction_list):
            decoded_boxlist = rbox_list.RBoxList(box_location)
    ...
    def _batch_decode(self, box_encodings):
        ...
        Returns:
          decoded_boxes: A float32 tensor of shape
            [batch_size, num_anchors, 5] containing the decoded boxes.
        """
        ...
        tiled_anchors_boxlist = rbox_list.RBoxList(tf.reshape(tiled_anchor_boxes, [-1, self._box_coder.code_size]))
        decoded_boxes = self._box_coder.decode(tf.reshape(box_encodings, [-1, self._box_coder.code_size]),
                                               tiled_anchors_boxlist)
        return tf.reshape(decoded_boxes.get(), tf.stack([combined_shape[0], combined_shape[1], 5]))
