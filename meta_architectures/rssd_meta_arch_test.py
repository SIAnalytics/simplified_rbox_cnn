...
"""Tests for object_detection.meta_architectures.rssd_meta_arch."""
import functools
import numpy as np
...
class MockAnchorGenerator2x2(anchor_generator.AnchorGenerator):
    ...
    def _generate(self, feature_map_shape_list):
        return rbox_list.RBoxList(
            tf.constant([[0, 0, .5, .5, 0],
                         [0, .5, .5, 1, 0],
                         [.5, 0, 1, .5, 0],
                         [.5, .5, 1, 1, 0]], tf.float32))


class RSsdMetaArchTest(tf.test.TestCase):
    def setUp(self):
        ...
        mock_rbox_predictor = test_utils.MockRBoxPredictor(is_training, self._num_classes)
        ...
        non_max_suppression_fn = functools.partial(post_processing_rbox.batch_multiclass_non_max_suppression_rbox,
                                                   score_thresh=-20.0,
                                                   iou_thresh=1.0,
        ...
        self._model = rssd_meta_arch.RSSDMetaArch(
            is_training, mock_anchor_generator, mock_rbox_predictor, mock_rbox_coder,
    ...
    def test_predict_results_have_correct_keys_and_shapes(self):
        ...
        input_shapes = [(batch_size, image_size, image_size, 3),
                        ...
                        (None, None, None, 3)]
        expected_box_encodings_shape_out = (batch_size, self._num_anchors, self._code_size)
        ...
        for input_shape in input_shapes:
            ...
            self.assertAllEqual(prediction_out['class_predictions_with_background'].shape,
    ...
    def test_postprocess_results_are_correct(self):
        ...
        expected_boxes = np.array([[[.5, .5, 1, 1, 0],
                                    [.5, 0, 1, .5, 0],
                                    [0, .5, .5, 1, 0],
                                    [0, 0, .5, .5, 0],
                                    [0, 0, 0, 0, 0]],
    ...
    def test_loss_results_are_correct(self):
        ...
        groundtruth_boxes_list = [tf.constant([[0, 0, .5, .5, 0]], dtype=tf.float32),
                                  tf.constant([[0, 0, .5, .5, 0]], dtype=tf.float32)]
