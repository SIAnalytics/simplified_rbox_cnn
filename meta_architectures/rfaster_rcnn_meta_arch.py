...
"""Rotated Faster R-CNN meta-architecture definition.
This code referred to 'faster_rcnn_meta_arch.py'.

General tensorflow implementation of Rotated Faster R-CNN detection models.
...
from core.rotated_subsampling import rotated_subsampling
from core.rotated_subsampling import rotated_subsampling_diagonal
from protos import rfaster_rcnn_pb2
from utils import shape_utils

slim = tf.contrib.slim


class RFasterRCNNMetaArch(model.DetectionModel):
    """Rotated Faster R-CNN Meta-architecture definition."""
    ...
    def __init__(self,
        ...
        """Rotated FasterRCNNMetaArch Constructor.
        ...
        """
        super(RFasterRCNNMetaArch, self).__init__(num_classes=num_classes)
        ...
        if not isinstance(first_stage_anchor_generator, grid_rbbox_anchor_generator.GridRbboxAnchorGenerator):
        ...
        self._proposal_target_assigner = target_assigner.create_target_assigner(
            'RFasterRCNN', 'proposal')
        self._detector_target_assigner = target_assigner.create_target_assigner(
            'RFasterRCNN', 'detection', unmatched_cls_target=unmatched_cls_target)
    ...
    def preprocess(self, inputs):
        ...
        with tf.name_scope('Preprocessor'):
            if self._image_resizer_fn is None:
    ...
    def predict(self, preprocessed_inputs):
        ...
        Returns:
          prediction_dict: a dictionary holding "raw" prediction tensors:
            ...
            6) anchors: A 2-D tensor of shape [num_anchors, 5] representing anchors
            ...
            7) refined_box_encodings: a 3-D tensor with shape
              ...
              (final) refined rotated box encodings, where
            ...
            10) proposal_boxes: A float32 tensor of shape
              [batch_size, self.max_num_proposals, 5] representing
              decoded proposal rotated bounding boxes (in absolute coordinates).
        ...
        if self._is_training:
            (rpn_box_encodings, rpn_objectness_predictions_with_background, anchors_boxlist) \
                = self._remove_invalid_anchors_and_predictions(rpn_box_encodings,
                                                               rpn_objectness_predictions_with_background,
                                                               anchors_boxlist,
                                                               clip_window)
        else:
            # RBBox isn't supported for clip_to_window
            # anchors_boxlist = rbox_list_ops.clip_to_window(anchors_boxlist, clip_window
            pass
        ...
        if not self._first_stage_only:
            prediction_dict.update(self._predict_second_stage(rpn_box_encodings,
                                                              rpn_objectness_predictions_with_background,
    ...
    def _predict_second_stage(self, rpn_box_encodings,
                              ...
                              anchors,
                              image_shape):
        """Predicts the output tensors from second stage of R-Faster R-CNN.
        ...
        Returns:
          prediction_dict: a dictionary holding "raw" prediction tensors:
            ...
            5) mask_predictions: (optional) a 4-D tensor with shape
              ...
              containing instance mask predictions.
        """
        proposal_rboxes_normalized, _, num_proposals = self._postprocess_rpn(rpn_box_encodings,
                                                                             rpn_objectness_predictions_with_background,
                                                                             anchors,
                                                                             image_shape)

        flattened_proposal_feature_maps = (self._compute_second_stage_input_feature_maps(rpn_features_to_crop,
                                                                                         proposal_rboxes_normalized))

        box_classifier_features = (
            self._feature_extractor.extract_box_classifier_features(flattened_proposal_feature_maps,
        ...
        absolute_proposal_boxes = rbox_list_ops.normalized_to_image_coordinates(proposal_rboxes_normalized,
                                                                                image_shape,
    ...
    def _remove_invalid_anchors_and_predictions(
        ...
        Returns:
          ...
          anchors: A BoxList representing num_valid_anchors anchors (for the RPN) in
            absolute coordinates.
        """
        pruned_anchors_boxlist, keep_indices = rbox_list_ops.prune_outside_window(anchors_boxlist, clip_window)
    ...
    def postprocess(self, prediction_dict):
        ...
        Returns:
          detections: a dictionary containing the following fields
            detection_boxes: [batch, max_detection, 5]
    ...
    def _postprocess_rpn(self,
        ...
        Args:
          ...
          anchors: A 2-D tensor of shape [num_anchors, 5] representing anchors
        ...
        Returns:
          proposal_boxes: A float tensor with shape
            [batch_size, max_num_proposals, 5] representing the (potentially zero
        ...
        clip_window = tf.to_float(tf.stack([0, 0, image_shape[1], image_shape[2]]))
        (proposal_boxes, proposal_scores, _, _, num_proposals) = \
            post_processing_rbox.batch_multiclass_non_max_suppression_rbox(
                ...
                self._first_stage_max_proposals,
                image_shape=image_shape,
                clip_window=clip_window,  # clip_to_window isn't implemented
                intersection_tf=True)
        if self._is_training:
            proposal_boxes = tf.stop_gradient(proposal_boxes)
            if not self._hard_example_miner:
                (groundtruth_rboxlists, groundtruth_classes_with_background_list,
        ...
        proposal_boxes_reshaped = tf.reshape(proposal_boxes, [-1, 5])
        normalized_proposal_boxes_reshaped = rbox_list_ops.to_normalized_coordinates(
            rbox_list.RBoxList(proposal_boxes_reshaped),
            image_shape[1], image_shape[2], check_range=False).get()
        proposal_boxes = tf.reshape(normalized_proposal_boxes_reshaped, [-1, proposal_boxes.shape[1].value, 5])
    ...
    def _unpad_proposals_and_sample_box_classifier_batch(
            ...
            proposal_scores,
            num_proposals,
            groundtruth_rboxlists,
            groundtruth_classes_with_background_list):
        """Unpads proposals and samples a minibatch for second stage.

        Args:
          proposal_rboxes: A float tensor with shape
            [batch_size, num_proposals, 5] representing the (potentially zero
            padded) proposal boxes for all images in the batch.  These boxes are
            represented as normalized coordinates.
          proposal_scores:  A float tensor with shape
          ...
          num_proposals: A Tensor of type `int32`. A 1-D tensor of shape [batch]
            representing the number of proposals predicted for each image in
            the batch.
          groundtruth_rboxlists: A list of BoxLists containing (absolute) coordinates
        ...
        Returns:
          proposal_boxes: A float tensor with shape
            [batch_size, second_stage_batch_size, 5] representing the (potentially
        ...
        for (single_image_proposal_rboxes,
             single_image_proposal_scores,
             single_image_num_proposals,
             single_image_groundtruth_rboxlist,
             single_image_groundtruth_classes_with_background) in zip(tf.unstack(proposal_rboxes),
                                                                      tf.unstack(proposal_scores),
                                                                      tf.unstack(num_proposals),
                                                                      groundtruth_rboxlists,
                                                                      groundtruth_classes_with_background_list):
            static_shape = single_image_proposal_rboxes.get_shape()
            sliced_static_shape = tf.TensorShape([tf.Dimension(None), static_shape.dims[-1]])
            single_image_proposal_rboxes = tf.slice(single_image_proposal_rboxes,
                                                    [0, 0],
                                                    [single_image_num_proposals, -1])
            single_image_proposal_rboxes.set_shape(sliced_static_shape)
            single_image_proposal_scores = tf.slice(single_image_proposal_scores,
                                                    [0],
                                                    [single_image_num_proposals])
            single_image_rboxlist = rbox_list.RBoxList(single_image_proposal_rboxes)
            single_image_rboxlist.add_field(fields.BoxListFields.scores,
                                            single_image_proposal_scores)
            sampled_boxlist = self._sample_box_classifier_minibatch(single_image_rboxlist,
                                                                    single_image_groundtruth_rboxlist,
                                                                    single_image_groundtruth_classes_with_background)
            sampled_padded_boxlist = rbox_list_ops.pad_or_clip_box_list(sampled_boxlist,
                                                                        num_boxes=self._second_stage_batch_size)
            single_image_num_proposals_sample.append(tf.minimum(sampled_boxlist.num_boxes(),
    ...
    def _format_groundtruth_data(self, image_shape):
        ...
        """
        groundtruth_rboxlists = [
            rbox_list_ops.to_absolute_coordinates(rbox_list.RBoxList(boxes), image_shape[1], image_shape[2])
        ...
        return groundtruth_rboxlists, groundtruth_classes_with_background_list
    ...
    def _sample_box_classifier_minibatch(self,
        ...
        sampled_indices = self._second_stage_sampler.subsample(tf.cast(cls_weights, tf.bool),
                                                               self._second_stage_batch_size,
                                                               positive_indicator)
        return rbox_list_ops.boolean_mask(proposal_boxlist, sampled_indices)
    ...
    def _compute_second_stage_input_feature_maps(self, features_to_crop,
        ...
        Args:
          ...
          proposal_rboxes_normalized: A float32 tensor with shape [batch_size,
        ...
        flat_proposal_rboxes_normalized = self._flatten_first_two_dimensions(proposal_rboxes_normalized)
        cropped_regions = rotated_subsampling(features_to_crop,
                                              flat_proposal_rboxes_normalized,
                                              get_box_inds(proposal_rboxes_normalized),
    ...
    def _postprocess_box_classifier(self,
        ...
        Args:
          refined_box_encodings: a 3-D tensor with shape
            [total_num_padded_proposals, num_classes, 5] representing predicted
            (final) refined rotated box encodings.
          ...
          proposal_boxes: [batch_size, self.max_num_proposals, 5] representing
            decoded proposal rotated bounding boxes.
          num_proposals: A Tensor of type `int32`. A 1-D tensor of shape [batch]
            representing the number of proposals predicted for each image in
            the batch.
          image_shape: a 1-D tensor representing the input image shape.
        ...
        class_predictions_batch = tf.reshape(tf.slice(class_predictions_with_background_batch, [0, 0, 1], [-1, -1, -1]),
        ...
        (nmsed_boxes, nmsed_scores, nmsed_classes, nmsed_masks, num_detections) = \
            self._second_stage_nms_fn(refined_decoded_boxes_batch,
    ...
    def _batch_decode_boxes(self, box_encodings, anchor_boxes):
        ...
        Returns:
          refined_box_predictions: a [batch_size, max_num_proposals, num_classes, 5]
        ...
        tiled_anchors_boxlist = rbox_list.RBoxList(tf.reshape(tiled_anchor_boxes, [-1, 5]))
        decoded_boxes = self._box_coder.decode(tf.reshape(box_encodings, [-1, self._box_coder.code_size]),
                                               tiled_anchors_boxlist)
        return tf.reshape(decoded_boxes.get(), tf.stack([combined_shape[0], combined_shape[1], num_classes, 5]))
    ...
    def loss(self, prediction_dict, scope=None):
        ...
        with tf.name_scope(scope, 'Loss', prediction_dict.values()):
            ...
            loss_dict = self._loss_rpn(prediction_dict['rpn_box_encodings'],
                                       prediction_dict['rpn_objectness_predictions_with_background'],
                                       prediction_dict['anchors'],
            ...
            if not self._first_stage_only:
                loss_dict.update(
                    self._loss_box_classifier(prediction_dict['refined_box_encodings'],
                                              prediction_dict['class_predictions_with_background'],
                                              prediction_dict['proposal_boxes'],
                                              prediction_dict['num_proposals'],
                                              groundtruth_boxlists,
    ...
    def _loss_rpn(self,
        ...
        with tf.name_scope('RPNLoss'):
            (batch_cls_targets, batch_cls_weights, batch_reg_targets, batch_reg_weights, _) = \
                target_assigner.batch_assign_targets(self._proposal_target_assigner,
                                                     rbox_list.RBoxList(anchors),
                                                     groundtruth_boxlists,
            ...
            def _minibatch_subsample_fn(inputs):
                ...
                return self._first_stage_sampler.subsample(tf.cast(cls_weights, tf.bool),
            ...
            loss_dict = {
                'first_stage_localization_loss': self._first_stage_loc_loss_weight * localization_loss,
    ...
    def _loss_box_classifier(self,
        ...
        with tf.name_scope('BoxClassifierLoss'):
            ...
            proposal_rboxlists = [rbox_list.RBoxList(proposal_boxes_single_image)
                                  for proposal_boxes_single_image in tf.unstack(proposal_boxes)]
            batch_size = len(proposal_rboxlists)

            (batch_cls_targets_with_background, batch_cls_weights, batch_reg_targets,
             batch_reg_weights, _) = target_assigner.batch_assign_targets(self._detector_target_assigner,
                                                                          proposal_rboxlists,
                                                                          groundtruth_boxlists,
                                                                          groundtruth_classes_with_background_list)
            ...
            refined_box_encodings_masked_by_class_targets = tf.boolean_mask(refined_box_encodings_with_background,
                                                                            tf.greater(flat_cls_targets_with_background,
                                                                                       0))
            ...
            second_stage_cls_losses = self._second_stage_classification_loss(class_predictions_with_background,
            ...
            if self._hard_example_miner:
                (second_stage_loc_loss, second_stage_cls_loss) = \
                    self._unpad_proposals_and_apply_hard_mining(proposal_rboxlists,
                                                                second_stage_loc_losses,
                                                                second_stage_cls_losses,
                                                                num_proposals)

            second_stage_loc_loss = tf.multiply(self._second_stage_loc_loss_weight, second_stage_loc_loss,
                                                name='loc_loss')
            loss_dict = {
                'second_stage_localization_loss': second_stage_loc_loss,
    ...
    def _unpad_proposals_and_apply_hard_mining(self,
        ...
        for (proposal_boxlist, single_image_loc_loss, single_image_cls_loss,
             single_image_num_proposals) in zip(proposal_boxlists,
                                                ...
                                                tf.unstack(num_proposals)):
            proposal_boxlist = rbox_list.RBoxList(tf.slice(proposal_boxlist.get(),
                                                           [0, 0],
                                                           [single_image_num_proposals,
                                                            -1]))
