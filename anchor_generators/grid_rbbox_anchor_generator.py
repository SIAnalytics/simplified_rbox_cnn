...
"""
Generates grid rotated anchors on the fly as used in Faster RCNN
This code referred to 'grid_anchor_generator.py'.
...
class GridRbboxAnchorGenerator(anchor_generator.AnchorGenerator):
    ...
    def name_scope(self):
        return 'GridRboxAnchorGenerator'
    ...
    def num_anchors_per_location(self):
        ...
        """
        return [len(self._scales) * len(self._aspect_ratios) * len(self._angles)]
    ...
    def _generate(self, feature_map_shape_list):
        ...
        Returns:
          boxes: a RBoxList holding a collection of N anchor boxes
        Raises:
...
def tile_anchors(grid_height,
    ...
    scale, aspect_ratios and angles arguments. For example, setting scales=[.1, .2, .2],
    aspect ratios = [2,2,1/2], angles = [0, pi/2, -pi/2] means that we create nine boxes:
    one with scale .1, aspect ratio 2, one with scale .2, aspect ratio 2, and one with scale .2
    and aspect ratio 1/2 and each angle add to boxes.  Each box is multiplied by "base_anchor_size" before
    ...
    Args:
      ...
      aspect_ratios: a 1-d (float) tensor representing the aspect ratio of each
        ...
        must be equal.
      angles: a 1-d (float) list representing the angles of each box in the basis set.
      base_anchor_size: base anchor size as [height, width]
    ...
    bbox_centers = shape_utils.repeat(bbox_centers, [n_angles, 1])
    bbox_sizes = shape_utils.repeat(bbox_sizes, [n_angles, 1])
    bbox_centers = tf.reshape(bbox_centers, [-1, 2])
    bbox_sizes = tf.reshape(bbox_sizes, [-1, 2])

    rbboxes = tf.concat([bbox_centers, bbox_sizes, bbox_angles], axis=1)

    return rbox_list.RBoxList(rbboxes)
