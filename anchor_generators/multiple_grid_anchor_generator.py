...
def create_ssd_anchors(num_layers=6,
    ...
    Args:
      ...
      num_aspect_ration_of_middle_scale: integer number of aspect rations in a middle scale
        if num_aspect_ration_of_middle_scale is 1, then it proceeds in original ssd way.
    ...
    for layer, scale, scale_next in zip(
        ...
        if layer == 0 and reduce_boxes_in_lowest_layer:
            ...
            for idx, aspect_ratio in enumerate(aspect_ratios):
                ...
                if num_aspect_ration_of_middle_scale == 1:
                    if aspect_ratio == 1.0:
                        layer_box_specs.append((np.sqrt(scale * scale_next), 1.0))
                elif idx < num_aspect_ration_of_middle_scale:
