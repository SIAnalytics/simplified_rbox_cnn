...
"""Generates grid rotated anchors on the fly corresponding to multiple CNN layers.
This code referred to 'multiple_grid_anchor_generator.py'

Generates grid rotated anchors on the fly corresponding to multiple CNN layers as
described in:
...
class MultipleGridRbboxAnchorGenerator(anchor_generator.AnchorGenerator):
    """Generate a grid of rotated anchors for multiple CNN layers."""
    ...
    def __init__(self,
                 ...
                 clip_window=None):
        """Constructs a MultipleGridRbboxAnchorGenerator.
        ...
        Args:
          ...
          clip_window: a tensor of shape [4] specifying a window to which all
            ...
            is performed. Notice that a rotated bounding box is't supported for clipping
    ...
    def name_scope(self):
        return 'MultipleGridRbboxAnchorGenerator'
    ...
    def num_anchors_per_location(self):
        ...
        """
        return [len(box_specs)*len(self._angles) for box_specs in self._box_specs]
    ...
    def _generate(self,
                  ...
                  anchor_strides=None,
                  anchor_offsets=None):
        """Generates a collection of rotated bounding boxes to be used as anchors.
        ...
        each angles(ex:3) add to each box thus end up with 6*8^2 + 9*4^2 = 528 anchors in total.
        The layout of the output anchors follows the order of how the grid sizes and box_specs are
        ...
        for grid_size, scales, aspect_ratios, stride, offset in zip(
                feature_map_shape_list, self._scales, self._aspect_ratios, anchor_strides, anchor_offsets):
            anchor_grid_list.append(
                grid_rbbox_anchor_generator.tile_anchors(
        ...
        stddevs_tensor = 0.01 * tf.ones([num_anchors, 5], dtype=tf.float32, name='stddevs')
        concatenated_anchors.add_field('stddev', stddevs_tensor)

        return concatenated_anchors


def create_rssd_anchors(num_layers=6,
                        min_scale=0.2,
                        max_scale=0.95,
                        ...
                        num_aspect_ration_of_middle_scale=1):
    """Creates MultipleGridRbboxAnchorGenerator for SSD anchors.

    This function instantiates a MultipleGridRbboxAnchorGenerator that reproduces
    ...
    MultipleGridRbboxAnchorGenerator object are always in normalized coordinates.
    ...
    Args:
      ...
      aspect_ratios: list or tuple of (float) aspect ratios to place on each
        grid point.
      angles: list or tuple of (float) angles to place on each grid point.
      base_anchor_size: base anchor size as [height, width].
      reduce_boxes_in_lowest_layer: a boolean to indicate whether the fixed 3
        boxes per location is used in the lowest layer.
      num_aspect_ration_of_middle_scale: integer number of aspect rations in a middle scale.
    ...
    for layer, scale, scale_next in zip(range(num_layers), scales[:-1], scales[1:]):
        ...
        if layer == 0 and reduce_boxes_in_lowest_layer:
            ...
            for idx, aspect_ratio in enumerate(aspect_ratios):
                ...
                if idx < num_aspect_ration_of_middle_scale:
                    layer_box_specs.append((np.sqrt(scale * scale_next), aspect_ratio))

        box_specs_list.append(layer_box_specs)
    return MultipleGridRbboxAnchorGenerator(box_specs_list, angles, base_anchor_size)
