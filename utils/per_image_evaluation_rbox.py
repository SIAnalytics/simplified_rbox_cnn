...
"""Evaluate Object Detection result on a single image for rbox.
...
class PerImageEvaluationRbox(object):
    ...
    def __init__(self,
        ...
        """Initialized PerImageEvaluationRbox by evaluation parameters.
    ...
    def compute_object_detection_metrics(self, detected_boxes, detected_scores,
        ...
        Args:
          detected_boxes: A float numpy array of shape [N, 5], representing N
              regions of detected object regions.
              Each row is of the format [cy, cx, h, w, ang]
          detected_scores: A float numpy array of shape [N, 1], representing
              the confidence scores of the detected N object instances.
          detected_class_labels: A integer numpy array of shape [N, 1], repreneting
              the class labels of the detected N object instances.
          groundtruth_boxes: A float numpy array of shape [M, 5], representing M
        ...
        Returns:
          ...
          detection_results: a list containing detection results appending a new detection results
        """
        scores, tp_fp_labels, detection_results = self._compute_tp_fp(detected_boxes,
                                                                      detected_scores,
                                                                      detected_class_labels,
                                                                      groundtruth_boxes,
                                                                      groundtruth_class_labels,
                                                                      groundtruth_is_difficult_lists)
        is_class_correctly_detected_in_image = self._compute_cor_loc(detected_boxes,
                                                                     detected_scores,
                                                                     detected_class_labels,
                                                                     groundtruth_boxes,
                                                                     groundtruth_class_labels)
        return scores, tp_fp_labels, is_class_correctly_detected_in_image, detection_results
    ...
    def _compute_cor_loc(self, detected_boxes, detected_scores,
        ...
        for i in range(self.num_groundtruth_classes):
            ...
            is_class_correctly_detected_in_image[i] = \
                self._compute_is_aclass_correctly_detected_in_image(detected_boxes_at_ith_class,
    ...
    def _compute_is_aclass_correctly_detected_in_image(self, detected_boxes, detected_scores, groundtruth_boxes):
        ...
        Args:
          detected_boxes: A numpy array of shape [N, 5] representing detected box coordinates
          detected_scores: A 1-d numpy array of length N representing classification score
          groundtruth_boxes: A numpy array of shape [M, 5] representing ground truth box coordinates
        ...
        if detected_boxes.size > 0:
            if groundtruth_boxes.size > 0:
                ...
                max_score_detected_box = np.expand_dims(detected_boxes[max_score_id, :], axis=0)
                iou = np_rbox_ops.iou(max_score_detected_box, groundtruth_boxes, ignore_large_than_pi_2=False)
    ...
    def _compute_tp_fp(self, detected_boxes, detected_scores,
        ...
        for i in range(self.num_groundtruth_classes):
            ...
            detected_boxes_at_ith_class = detected_boxes[(detected_class_labels == i), :]
            detected_scores_at_ith_class = detected_scores[detected_class_labels == i]
            scores, tp_fp_labels, is_gt_boxes_detected = \
                self._compute_tp_fp_for_single_class(detected_boxes_at_ith_class,
                                                     detected_scores_at_ith_class,
                                                     gt_boxes_at_ith_class,
            ...
            if self.is_save_detection_results:
                for box, score, tp_fp_label in zip(detected_boxes_at_ith_class, detected_scores_at_ith_class,
                                                   tp_fp_labels):
                    if score > 0:
                        detection_result = dict()
                        detection_result['box'] = box
                        detection_result['score'] = score
                        detection_result['class_id'] = i
                        detection_result['match'] = 'TP' if tp_fp_label else 'FP'
                        detection_results.append(detection_result)
                for gt_box, is_gt_box_detected, is_difficult in zip(gt_boxes_at_ith_class,
                                                                    is_gt_boxes_detected,
                                                                    groundtruth_is_difficult_list_at_ith_class):
                    if not is_gt_box_detected and not is_difficult:
                        detection_result = dict()
                        detection_result['box'] = gt_box
                        detection_result['score'] = 0.0
                        detection_result['class_id'] = i
                        detection_result['match'] = 'FN'
                        detection_results.append(detection_result)

        return result_scores, result_tp_fp_labels, detection_results
    ...
    def _compute_tp_fp_for_single_class(self, detected_boxes, detected_scores,
        ...
        Args:
          ...
          detected_scores: A 1-d numpy array of length N representing classification
              score
          groundtruth_boxes: A numpy array of shape [M, 5] representing ground truth
              box coordinates
        ...
        if groundtruth_boxes.size == 0:
            ...
            is_gt_box_detected = np.zeros(groundtruth_boxes.shape[0], dtype=bool)

        if detected_boxes.size == 0:
            return np.array([], dtype=float), np.array([], dtype=bool), is_gt_box_detected

        detected_boxes, indices = np_rbox_ops.sort_by_score(detected_boxes, detected_scores)
        scores = detected_scores[indices]
        if groundtruth_boxes.size == 0:
            return scores, np.zeros(detected_boxes.shape[0], dtype=bool), is_gt_box_detected

        iou = np_rbox_ops.iou(detected_boxes, groundtruth_boxes, ignore_large_than_pi_2=False)
        max_overlap_gt_ids = np.argmax(iou, axis=1)
        tp_fp_labels = np.zeros(detected_boxes.shape[0], dtype=bool)
        is_matched_to_difficult_box = np.zeros(detected_boxes.shape[0], dtype=bool)
        ...
        return scores[~is_matched_to_difficult_box], tp_fp_labels[~is_matched_to_difficult_box], is_gt_box_detected
