...
class ObjectDetectionEvaluationRboxTest(tf.test.TestCase):

    def setUp(self):
        num_groundtruth_classes = 3
        self.od_eval = object_detection_evaluation.ObjectDetectionEvaluation(num_groundtruth_classes, box_type='rbox')

        image_key1 = "img1"
        groundtruth_boxes1 = np.array([[0, 0, 1, 1, 0], [0, 0, 2, 2, 0], [0, 0, 3, 3, 0]], dtype=np.float32)
        groundtruth_class_labels1 = np.array([0, 2, 0], dtype=int)
        self.od_eval.add_single_ground_truth_image_info(image_key1, groundtruth_boxes1, groundtruth_class_labels1)

        image_key2 = "img2"
        groundtruth_boxes2 = np.array([[10, 10, 11, 11, 0], [500, 500, 510, 510, 0], [10, 10, 12, 12, 0]], dtype=np.float32)
        groundtruth_class_labels2 = np.array([0, 0, 2], dtype=int)
        groundtruth_is_difficult_list2 = np.array([False, True, False], dtype=bool)
        self.od_eval.add_single_ground_truth_image_info(image_key2,
                                                        groundtruth_boxes2,
                                                        groundtruth_class_labels2,
                                                        groundtruth_is_difficult_list2)
        image_key3 = "img3"
        groundtruth_boxes3 = np.array([[0, 0, 1, 1]], dtype=np.float32)
        groundtruth_class_labels3 = np.array([1], dtype=int)
        self.od_eval.add_single_ground_truth_image_info(image_key3, groundtruth_boxes3, groundtruth_class_labels3)

        image_key = "img2"
        detected_boxes = np.array([[10, 10, 11, 11, 0], [100, 100, 120, 120, 0], [100, 100, 220, 220, 0]],
                                  dtype=np.float32)
        detected_class_labels = np.array([0, 0, 2], dtype=int)
        detected_scores = np.array([0.7, 0.8, 0.9], dtype=float)
    ...
    def test_add_single_ground_truth_image_info(self):
        ...
        groundtruth_boxes2 = np.array([[10, 10, 11, 11, 0], [500, 500, 510, 510, 0], [10, 10, 12, 12, 0]],
                                      dtype=np.float32)
    ...
    def test_add_single_detected_image_info(self):
        ...
        for i in range(self.od_eval.num_class):
            for j in range(len(expected_scores_per_class[i])):
                ...
                self.assertTrue(np.array_equal(expected_tp_fp_labels_per_class[i][j],
                                               self.od_eval.tp_fp_labels_per_class[i][j]))
        self.assertTrue(np.array_equal(expected_num_images_correctly_detected_per_class,
                                       self.od_eval.num_images_correctly_detected_per_class))

    def test_evaluate(self):
        (average_precision_per_class, mean_ap, precisions_per_class, recalls_per_class, corloc_per_class, mean_corloc)\
            = self.od_eval.evaluate()
        expected_precisions_per_class = [np.array([0, 0.5], dtype=float),
                                         np.array([], dtype=float),
                                         np.array([0], dtype=float)]
        expected_recalls_per_class = [np.array([0, 1. / 3.], dtype=float),
                                      np.array([], dtype=float),
                                      np.array([0], dtype=float)]
        expected_average_precision_per_class = np.array([1. / 6., 0, 0], dtype=float)
        expected_corloc_per_class = np.array([0, 0, 0], dtype=float)
        expected_mean_ap = 1. / 18
        expected_mean_corloc = 0.0
        for i in range(self.od_eval.num_class):
