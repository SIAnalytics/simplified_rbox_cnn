...
class VisualizationUtilsTest(tf.test.TestCase):
    ...
    def test_draw_bounding_rbox_on_image(self):
        test_image = self.create_colorful_test_image()
        test_image = Image.fromarray(test_image)
        width_original, height_original = test_image.size
        ymin = 0.25
    ...
    def test_draw_bounding_rbox_on_image_array(self):
        test_image = self.create_colorful_test_image()
        width_original = test_image.shape[0]
        height_original = test_image.shape[1]
        cy = 0.25
        ...
        visualization_utils.draw_bounding_rbox_on_image_array(test_image, cy, cx, h, w, ang)
        width_final = test_image.shape[0]
        height_final = test_image.shape[1]
    ...
    def test_draw_bounding_rboxes_on_image(self):
        test_image = self.create_colorful_test_image()
        test_image = Image.fromarray(test_image)
        width_original, height_original = test_image.size
        rboxes = np.array([[0.25, 0.75, 0.4, 0.6, 0.1],
                          [0.1, 0.1, 0.9, 0.9, 0.2]])

        visualization_utils.draw_bounding_rboxes_on_image(test_image, rboxes)
        width_final, height_final = test_image.size

        self.assertEqual(width_original, width_final)
        self.assertEqual(height_original, height_final)

    def test_draw_bounding_rboxes_on_image_array(self):
        test_image = self.create_colorful_test_image()
        width_original = test_image.shape[0]
        height_original = test_image.shape[1]
        rboxes = np.array([[0.25, 0.75, 0.4, 0.6, 0.1],
        ...
        visualization_utils.draw_bounding_rboxes_on_image_array(test_image, rboxes)
        width_final = test_image.shape[0]
        height_final = test_image.shape[1]
