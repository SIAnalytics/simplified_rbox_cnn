...
class MockRBoxCoder(box_coder.BoxCoder):
    """Simple RBoxCoder."""
    ...
    def _encode(self, rboxes, anchors):
        return rboxes.get() - anchors.get()

    def _decode(self, rel_codes, anchors):
        return rbox_list.RBoxList(rel_codes + anchors.get())


class MockRBoxPredictor(box_predictor.BoxPredictor):
    """Simple box predictor that ignores inputs and outputs all zeros."""

    def __init__(self, is_training, num_classes):
        super(MockRBoxPredictor, self).__init__(is_training, num_classes)
