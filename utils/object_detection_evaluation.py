...
Note: This module operates on numpy boxes and box lists.
      Added box_type in ObjectDetectionEvaluation to support rbox.
"""
...
class ObjectDetectionEvaluation(object):
    ...
    def __init__(self,
        ...
        if box_type == 'rbox':
            self.per_image_eval = per_image_evaluation_rbox.PerImageEvaluationRbox(
                num_groundtruth_classes, matching_iou_threshold, is_save_detection_results)
        else:  # box_type == 'box'
            self.per_image_eval = per_image_evaluation.PerImageEvaluation(
                num_groundtruth_classes, matching_iou_threshold, nms_iou_threshold, nms_max_output_boxes)
    ...
    def add_single_ground_truth_image_info(self,
        ...
        Args:
          ...
          groundtruth_boxes: A numpy array of shape [M, 4] representing object box
              ...
              or A numpy array of shape [M, 5] representing object box
              coordinates[cy, cx, h, w, ang]
    ...
    def add_single_detected_image_info(self, image_key, detected_boxes,
        ...
        if image_key in self.groundtruth_boxes:
            ...
            if self.box_type == 'rbox':
                groundtruth_boxes = np.empty(shape=[0, 5], dtype=float)
            else:  # box_type == box
                groundtruth_boxes = np.empty(shape=[0, 4], dtype=float)
            groundtruth_class_labels = np.array([], dtype=int)
            groundtruth_is_difficult_list = np.array([], dtype=bool)

        scores, tp_fp_labels, is_class_correctly_detected_in_image, detection_result = (
    ...
    def add_single_image_and_info(self, image, sensor='', filename=''):
        self.detection_results[-1] = {'image': image,
                                      'sensor': sensor,
                                      'filename': filename,
                                      'det': self.detection_results[-1]}
    ...
    def save_detection_results(self, filename):
        with open(filename, 'wb') as f:
            pickle.dump(self.detection_results, f)
