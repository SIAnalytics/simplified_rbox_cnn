...
class AnchorGeneratorBuilderTest(tf.test.TestCase):
    ...
    def test_build_rssd_anchor_generator_with_defaults(self):
        anchor_generator_text_proto = """
        rssd_anchor_generator {
          aspect_ratios: [1.0]
        }
      """
        anchor_generator_proto = anchor_generator_pb2.AnchorGenerator()
        ...
        self.assertTrue(isinstance(anchor_generator_object,
                                   multiple_grid_rbbox_anchor_generator.MultipleGridRbboxAnchorGenerator))
    ...
    def test_build_rssd_anchor_generator_withoud_reduced_boxes(self):
        anchor_generator_text_proto = """
        rssd_anchor_generator {
          aspect_ratios: [1.0]
          reduce_boxes_in_lowest_layer: false
    ...
    def test_build_rssd_anchor_generator_with_non_default_parameters(self):
        anchor_generator_text_proto = """
        rssd_anchor_generator {
          num_layers: 2
          min_scale: 0.3
      ...
      """
        ...
        self.assert_almost_list_equal(anchor_generator_object._angles, [0.1])

        with self.test_session() as sess:
            base_anchor_size = sess.run(anchor_generator_object._base_anchor_size)
        self.assertAllClose(base_anchor_size, [1.0, 1.0])

    def test_build_grid_rbbox_anchor_generator_with_defaults(self):
        anchor_generator_text_proto = """
        ...
        anchor_generator_object = anchor_generator_builder.build(anchor_generator_proto)
        self.assertTrue(isinstance(anchor_generator_object,
                                   grid_rbbox_anchor_generator.GridRbboxAnchorGenerator))
        self.assertListEqual(anchor_generator_object._scales, [])
        self.assertListEqual(anchor_generator_object._aspect_ratios, [])
        with self.test_session() as sess:
            base_anchor_size, anchor_offset, anchor_stride = sess.run(
    ...
    def test_build_grid_rbbox_anchor_generator_with_non_default_parameters(self):
        anchor_generator_text_proto = """
          grid_rbbox_anchor_generator {
            height: 128
            width: 512
        ...
        self.assert_almost_list_equal(anchor_generator_object._aspect_ratios,
                                      [0.3, 4.5])
        self.assert_almost_list_equal(anchor_generator_object._angles,
                                      [0.1, 0.2])
        with self.test_session() as sess:
            base_anchor_size, anchor_offset, anchor_stride = sess.run(
                [anchor_generator_object._base_anchor_size,
