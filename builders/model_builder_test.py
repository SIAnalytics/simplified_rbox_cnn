...
class ModelBuilderTest(tf.test.TestCase):
    ...
    def test_create_rssd_inception_v2_model_from_config(self):
        model_text_proto = """
      rssd {
        feature_extractor {
          type: 'ssd_inception_v2'
        ...
        text_format.Merge(model_text_proto, model_proto)
        model = self.create_model(model_proto)
        self.assertIsInstance(model, rssd_meta_arch.RSSDMetaArch)
        self.assertIsInstance(model._feature_extractor,
                              SSDInceptionV2FeatureExtractor)

    def test_create_rfaster_rcnn_resnet_v1_models_from_config(self):
        model_text_proto = """
          rfaster_rcnn {
            num_classes: 3
            image_resizer {
        ...
        text_format.Merge(model_text_proto, model_proto)
        for extractor_type, extractor_class in FEATURE_EXTRACTOR_MAPS.items():
            model_proto.rfaster_rcnn.feature_extractor.type = extractor_type
            model = model_builder.build(model_proto, is_training=True)
            self.assertIsInstance(model, rfaster_rcnn_meta_arch.RFasterRCNNMetaArch)
            self.assertIsInstance(model._feature_extractor, extractor_class)
