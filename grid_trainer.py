...
def _create_losses_rbbox(input_queue, create_model_fn):
    ...
    #-------------------------------------------------------------
    # def pyfunc(images, groundtruth_rboxes_list):
    #     import pickle
    #     global _step
    #     data = {'images': images,
    #             'groundtruth_rboxes_list': groundtruth_rboxes_list}
    #     with open('../input/' + str(_step) + '.pkl', 'wb') as f:
    #         pickle.dump(data, f)
    #         _step = _step + 1
    #     return 0
    # from utils import shape_utils
    # pad_groundtruth_rboxes_list = [shape_utils.pad_tensor(gt, 20) for gt in groundtruth_rboxes_list]
    # _pyfunc = tf.py_func(pyfunc, [images, pad_groundtruth_rboxes_list], tf.int64)
    # with tf.control_dependencies([_pyfunc]):
